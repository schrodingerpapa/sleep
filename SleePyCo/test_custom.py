...
class OneFoldTrainer:
    ...
    def build_model(self):
        ...
        load_path = os.path.join(
            "checkpoints", self.cfg["name"], "ckpt_fold-{0:02d}.pth".format(self.fold)
        )
    ...
    def run_csv(self, csv_path):
        ...
        data = (
            pd.read_csv(csv_path, header=0).values[:, 1].squeeze() * 2
        )  # shape: (总点数,)
        ...
        for i in range(num_segments):
            ...
            frames = [
                segment[j * frame_length : (j + 1) * frame_length] for j in range(10)
            ]  # 拆成 10 帧
        ...
        df = pd.DataFrame(
            {"Frame": range(1, len(predictions) + 1), "Predicted": predictions}
        )
        filename = "predictions-ads"
        save_path = r"/home/chenlungan/算法模型/SleePyCo/results/{filename}.csv".format(
            filename=filename
        )
    ...
    def run_npz(self, npz_path):
        ...
        for i in range(num_segments):
            ...
            frames = [
                segment[j * 3000 : (j + 1) * 3000] for j in range(10)
            ]  # 拆成 10 帧
            input_frames = np.array(frames)[:, np.newaxis, :]  # shape: (10, 1, 3000)
            inputs = torch.tensor(input_frames, dtype=torch.float32).to(self.device)
    ...
    def run_ensemble(self, npz_path):
        ...
        for fold in range(1, 11):  # 遍历所有 fold
            ...
            self.model = self.build_model()
            load_path = os.path.join(
                "checkpoints", self.cfg["name"], f"ckpt_fold-{fold:02d}.pth"
            )
            ...
            # 计算总耗时
            total_time = sum((end_time - start_time) for fold_preds in all_predictions)
        ...
        df = pd.DataFrame(
            {
                ...
                "real label": label_data[
                    : len(final_prediction)
                ],  # 假设标签为0，实际使用时请替换为真实标签
            }
        )
        df.to_csv(
            r"/home/chenlungan/算法模型/SleePyCo/results/SC_predictions.csv",
            index=False,
        )
...
def main():
    ...
    parser.add_argument(
        "--csv", type=str, default=None, help="path to csv file for evaluation"
    )
    parser.add_argument(
        "--npz", type=str, default=None, help="path to npz file for evaluation"
    )
    parser.add_argument(
        "--ensemble", action="store_true", help="使用 20 折模型集成推理"
    )
