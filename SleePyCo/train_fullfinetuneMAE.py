...
class OneFoldTrainer:
    ...
    def build_model(self):
        ...
        if self.tp_cfg["mode"] != "scratch":
            print("[INFO] Model loaded for fullfinetuneMAE")
            # load_name = load_name.replace(
            #     "SL-{:02d}".format(self.ds_cfg["seq_len"]), "SL-01"
            # )
            
            load_name = load_name.replace(self.tp_cfg["mode"], "freezefinetune")
    ...
    def activate_train_mode(self):
        ...
        if self.tp_cfg["mode"] == "fullfinetuneMAE":
            print("[INFO] Full finetune: all parameters trainable")
            # 确保所有参数可训练（可能之前被冻结）
            for p in raw_model.parameters():
                p.requires_grad = True
            # 可选：将编码器的 BatchNorm 设回训练模式（如果之前被设为 eval）
    ...
    def train_one_epoch(self, epoch):
        ...
        for i, (inputs, labels) in enumerate(self.loader_dict["train"]):
            ...
            outputs = self.model(inputs) # shape为[B, num_classes]
    ...
    def evaluate(self, mode):
        ...
        if mode == "val":
            return 100.0 * correct / total, eval_loss
        elif mode == "test":
