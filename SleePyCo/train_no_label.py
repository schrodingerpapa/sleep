...
class OneFoldTrainer:
    def __init__(self, args, fold, config):
        ...
        self.writer = SummaryWriter(
            log_dir=os.path.join("runs", config["name"], f"fold_{fold}")
        )
    ...
    def train_one_epoch(self):
        ...
        for i, (inputs, labels) in enumerate(self.loader_dict["train"]):
            ...
            inputs = torch.cat([inputs[0], inputs[1]], dim=0).to(
            ...
            if i % 10 == 0:  # 每10个batch更新一次
                progress_bar(
                    ...
                    "Lr: %.4e | Loss: %.3f"
                    % (get_lr(self.optimizer), train_loss / (i + 1)),
                )
            ...
            if self.train_iter % self.tp_cfg["val_period"] == 0:
                ...
                self.writer.add_scalar(
                    "Loss/Train", train_loss / num_batches, self.train_iter
                )
                self.writer.add_scalar("Loss/Validation", val_loss, self.train_iter)
                self.writer.add_scalar(
                    "Learning Rate", get_lr(self.optimizer), self.train_iter
                )
            ...
            for i, (inputs, labels) in enumerate(self.loader_dict[mode]):
                ...
                if i % 10 == 0:  # 每10个batch更新一次
                    progress_bar(
                        ...
                        "Lr: %.4e | Loss: %.3f"
                        % (get_lr(self.optimizer), eval_loss / (i + 1)),
                    )
