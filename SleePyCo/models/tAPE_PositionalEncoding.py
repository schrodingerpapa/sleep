...
class tAPE(nn.Module):
    def __init__(self, config, emb_size, dropout=0.1):
        ...
        # tAPE 缩放因子：(out_features / self.max_len)
        scale_factor = emb_size / self.max_len
        pe[:, 0::2] = torch.sin(position * div_term * scale_factor)  # 偶数维度
        pe[:, 1::2] = torch.cos(position * div_term * scale_factor)  # 奇数维度
        # ---------------------------------------------------------------------------
...
#             self.max_len = 5000
#         self.num_heads = num_heads
#         self.scale = emb_size ** -0.5 # 缩放因子
#         # self.to_qkv = nn.Linear(inp, inner_dim * 3, bias=False)
#         self.key = nn.Linear(emb_size, emb_size, bias=False) # 线性投影层
#         self.value = nn.Linear(emb_size, emb_size, bias=False)
#         self.query = nn.Linear(emb_size, emb_size, bias=False)

#         self.relative_bias_table = nn.Parameter(torch.zeros((2 * self.max_len - 1), num_heads)) # 定义相对位置表，可学习参数
#         coords = torch.meshgrid((torch.arange(1), torch.arange(self.max_len)))
#         coords = torch.flatten(torch.stack(coords), 1)
#         relative_coords = coords[:, :, None] - coords[:, None, :]
#         relative_coords[1] += self.max_len - 1
...
#         relative_bias = rearrange(relative_bias, '(h w) c -> 1 c h w', h=1 * seq_len, w=1 * seq_len)
...
#         out = out.reshape(batch_size, seq_len, -1)
#         # out.shape == (batch_size, seq_len, d_model)
#         out = out.transpose(0,1)  # B,L,C -> L,B,C
#         return out
class Attention_Rel_Scl(nn.Module):
    def __init__(self, config, emb_size, num_heads, dropout):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 计算相对位置: 对于每个位置i和j，相对位置是 i-j
        relative_position = position_index.unsqueeze(1) - position_index.unsqueeze(
            0
        )  # shape: (seq_len, seq_len)
        # 转换为相对位置表的索引 (从0到2*max_len-2)
        ...
        # 从相对位置表中获取对应的位置偏差
        # relative_position_index shape: (seq_len, seq_len)
        # self.relative_bias_table shape: (2*max_len-1, num_heads)
        relative_bias = self.relative_bias_table[
            relative_position_index
        ]  # shape: (seq_len, seq_len, num_heads)
        # 调整维度以匹配注意力分数
        relative_bias = relative_bias.permute(2, 0, 1).unsqueeze(
            0
...
class eRPE_Transformer(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(0, 1)  # 转置以匹配位置编码的形状, B,L,C->L,B,C
        x_src = self.embed_layer(x)
        if self.Fix_pos_encode != "None":
            x_src = self.Fix_Position(x_src)
        out = self.transformer(x_src)
        out = out.transpose(0, 1)  # 转置回原始形状, L,B,C->B,L,C

        if self.pool == "mean":
            out = out.mean(dim=1)  # 输入 (B, L, C) → 输出 (B, C)
        elif self.pool == "last":
            out = out[:, -1]  # 输入 (B, L, C) → 输出 (B, C)
        elif self.pool == "attn":
            a_states = torch.tanh(
                self.w_ha(out)
            )  # (B, L, C) → (B, L, C)（线性变换+激活）
