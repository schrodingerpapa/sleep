...
def main():
    ...
    psg_fnames = glob.glob(os.path.join(args.data_dir, "*PSG.edf"))  # 查找所有以PSG.edf结尾的文件
    ann_fnames = glob.glob(os.path.join(args.data_dir, "*Hypnogram.edf"))  # 查找所有以Hypnogram.edf结尾的文件
    psg_fnames.sort()  # 按文件名排序
    ann_fnames.sort()  # 按文件名排序
    psg_fnames = np.asarray(psg_fnames)  # 将文件名列表转换为numpy数组
    ...
    for i in range(len(psg_fnames)):
        ...
        assert psg_f.getStartdatetime() == ann_f.getStartdatetime()  # 调试，确保信号文件和注释文件的开始时间相同
        ...
        signals = psg_f.readSignal(select_ch_idx).reshape(-1, n_epoch_samples)  # -1表示根据n_epoch_samples自动计算行数,每一行代表一个epoch
        ...
        w_edge_mins = 30  # 只选择睡眠期的前后30分钟
        nw_idx = np.where(y != stage_dict["W"])[0]  # 找到所有非睡眠期的索引
        start_idx = nw_idx[0] - (w_edge_mins * 2)  # 选择睡眠期前30分钟的数据，每个索引指标代表一个epoch30s时长
        end_idx = nw_idx[-1] + (w_edge_mins * 2)  # 选择睡眠期后30分钟的数据
        if start_idx < 0: start_idx = 0  # 如果开始索引小于0，则设置为0
