...
# 加载损失数据（包含字典的数组）
losses = np.load('losses_fold-01.npy', allow_pickle=True).item()  # .item() 将数组转换为字典

# 提取训练损失和验证损失（直接通过键访问）
train_losses = losses['train_losses']  # 训练损失数组
val_losses = losses['val_losses']      # 验证损失数组
train_time = losses['training_time'] # 训练时长
...
# # 保存图片
plt.savefig('loss_curve_fold-01.png', dpi=300, bbox_inches='tight')
