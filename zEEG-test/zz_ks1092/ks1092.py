...
def visualize_eeg(data: np.ndarray, metadata: Dict[str, str], title: str = "EEG Data", start_time=None, end_time=None):
    sampling_rate = metadata.get('sampling_rate', 500.0)
    channel_names = metadata.get('channels', ['原始数据', '嵌入式滤波后数据'])
...
def filter_eeg(data: np.ndarray, sampling_rate, low_cut, high_cut,
               notch) -> np.ndarray:
    """
    对脑电数据进行滤波处理
...
def plot_band_power(eeg_data, fs=500):
    """绘制各频段能量分布，支持多通道数据"""

    # 检查数据是否为多通道
    if eeg_data.ndim > 1 and eeg_data.shape[1] > 1:
        n_channels = eeg_data.shape[1]
        channel_names = ['原始数据', '嵌入式滤波后数据'][:n_channels]
...
def main():
    """主函数，演示如何使用上述函数"""
    file_path = r"C:\Users\clg\Desktop\开发板静息闭眼\2datafile.txt"
    ...
    try:
        ...
        if start_time is not None and end_time is not None:
            ...
            data_selected = data[start_idx:end_idx]
            freqs, psd = compute_spectrum(data_selected, metadata['sampling_rate'])
        else:
            freqs, psd = compute_spectrum(data, metadata['sampling_rate'])
        visualize_spectrum(freqs, psd, metadata)

        # 数据滤波
        print("\n正在进行信号滤波...")
        filtered_data = filter_eeg(data, sampling_rate, low_cut, high_cut,notch)

        # 可视化滤波后的数据
        print("\n显示滤波后的数据...")
        visualize_eeg(filtered_data, metadata, title="滤波后脑电数据", start_time=start_time, end_time=end_time)
        ...
        # if start_time is not None and end_time is not None:
        #     filtered_data_selected = filtered_data[start_idx:end_idx]
        #     plot_band_power(filtered_data_selected, fs=500)
        # else:
        #     plot_band_power(filtered_data, fs=500)
