    ...
    try:
        ...
        if file_ext in ['.xlsx', '.xls']:
            ...
            for enc in ['utf-8', 'gbk', 'utf-8-sig', 'latin1']:
                try:
                    data = pd.read_csv(file_path, header=None, on_bad_lines='skip', encoding=enc)
                    break
                except Exception as e:
                    print(f"尝试编码 {enc} 失败: {e}")
            else:
                print("所有常用编码均无法读取该文件，请检查文件格式。")
                return None
        last_column_index = data.shape[1] - 1
        eeg_data = data.iloc[1:, last_column_index]  # 跳过第一行
        eeg_data = pd.to_numeric(eeg_data, errors='coerce')
        eeg_data = eeg_data.dropna()
        print(f"成功加载数据，共 {len(eeg_data)} 个样本 ({len(eeg_data) / SAMPLING_RATE:.2f}秒)")
        return eeg_data.values
    except Exception as e:
        print(f"加载数据失败: {e}")
        return None
...
def main():
    ...
    # 加载数据
    file_path = r"C:\研究生\睡眠硬件--采集测试\测试数据\芯聚威测试\1Hz-20uv-x12-wulvbo.csv"
